...
- [5.0.wavenumber_spectrum.py](#5-wavenumber-spectrum) uses the processed data pairs and compute the wavenumber spectrum.
...
1. The output figures are saved in the `figures/` directory:
...
1. Use photo editor such as Gimp to create the Figure 1 in Wang et al. (2025)
...
This script calculates the density from moorings and gliders, combines them into one file, removes outliers, and saves the resulting data. The density anomaly was calculated against the full-depth domain mean calculated using the 4 S-moorings and provided in `data/density_timemean.nc`. 

Unless you want to recaculate from the source data on PO.DAAC, this script can be skipped. The data produced by this script is the fundation of all following analysis and can be downloaded from the AGU website. Details are included in the Supplemental Materials.
...
- Open input files and extract relevant variables (temperature, salinity, pressure)
- Calculate seawater density using the `c_rho` function
- Create a new dataset with the calculated densities

1. **Gliders**

- Open input files and extract relevant variables (pressure, temperature, salinity)
...
- Save the combined dataset to disk as a new netCDF file (`fn_out=data/mooring.data/density_all_moorings_level-2_removed_outliers.nc`)
...
- Steric height
  - Produced by the second step [2.0.calculate_steric_height.py](#2-calculate-steric-height).
...
- **colocate(karin_fn, steric)**
  ...
  - `steric`: Steric height dataset from moorings and gliders. These data are produced by [2.0.calculate_steric_height.py](#2-calculate-steric-height).
...
```markdown
    Parameters:
    bottom_depth   : Integration depth at the bottom of the water column (in meters).
    top_depth      : Integration depth at the top of the water column (in meters).
    deltat_threshold    : Maximum allowable time difference between steric (mooring profile) and SWOT flyover time (in minutes).
    valid_points      : Minimum number of valid points required to remove trends. Default is 5; 
...
1. **select_colocated_data(dd, max_deltat_left, max_deltat_right, distance_to_center)**
...
1. **plot_locations(dd, max_deltat_left=60, max_deltat_right=60, distance_to_center=0.02)**
...
1. **plot_latitudes(dd, max_deltat_left=60, max_deltat_right=60, distance_to_center=0.025)**
...
1. **plot_profiles_space(dd, var_name='steric_linear')**
...
1. **plot_time_series(data0, var_name)**
...
1. **error_stats(data, var_name, plotit=False)**
...
1. **loop_over_parameters_for_stats()**

...
def plot_profiles_space(dd,var_name='steric_linear'):
    ...
    nn=len(time)
    n_row=nn//10+1 
    fig,ax=plt.subplots(n_row,10,figsize=(14,10),sharex=True,sharey=True,gridspec_kw={'hspace': 0.1, 'wspace': 0.1})
    axs=ax.flatten()
    for i in range(nn):
        ...
        #axs[i].set_title(time[i])
        #axs[i].set_xticks([])
        axs[i].set_title('')
        axs[i].grid(True)
        axs[i].set_ylim(-3,3)
    ax[n_row//2,0].set_ylabel('ssha (cm)')
    ax[-1,5].set_xlabel('Latitude')
    ax[0,5].set_title('Steric height (blue) and SWOT (red) spatial profiles')
    #plt.tight_layout()
...
def error_stats(data,var_name,plotit=False):
    """
...
def loop_over_parameters_for_stats():
    """ 
    Loop over the parameters for the colocated data statistics.
    """
    for depth in [-500,-300]:
        for valid_points in [6,9]: #minimum number of valid points to remove trend, default is 5, less than 5 points will be dropped
            for deltat_threshold in [20,30,60,120]: # maximum time difference between steric and karin in minutes
                ...
                fn_in=f'../data/3.0.colocated_data_karin_moorings_gliders_depth_{-depth:3d}.csv'
                ...
                ff.write(f'number of data points: {len(data0)}\n')
                stats=error_stats(data0,var_name,)
                print(f'{data0['ssha_karin'].var()**0.5:.3f}\n')
                print(f'{depth}',
                      f'& {valid_points}',
                      f'& {deltat_threshold}',
                      f'& {stats['total'].values[0]:.3f}',
                      f'& {stats['total'].values[1]:.3f}',
                      f'& {len(data0):.0f}',
                      f'& {len(data0['time_karin'].unique()):.0f} \\\\')
...
if __name__=='__main__':
    ...
    #uncommet the following line to loop over the parameters for the sensitivity analysis presented in the Supplementary Material
    #loop_over_parameters_for_stats()
